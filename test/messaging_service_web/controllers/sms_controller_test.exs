defmodule MessagingServiceWeb.SmsControllerTest do
  ...
  describe "incoming message" do
    test "succeeds", %{conn: conn} do
      conn = post(conn, ~p"/api/messages/sms", sms_attrs())
      assert response(conn, 200)
    end

    test "creates a new conversation", %{conn: conn} do
      attrs = sms_attrs() |> Map.put("from", UUID.generate())
      conn = post(conn, ~p"/api/messages/sms", attrs)
      assert response(conn, 200)
  ...
  test "outgoing message", %{conn: conn} do
    conn = post(conn, ~p"/api/webhooks/sms", sms_attrs())
    assert response(conn, 200)
  end
end
